...
class TestAttackForecast:
    async def test_forecast_attack_returns_damage_counter_and_chinese_description(self, client):
        gid, human, _ai = await _make_game(client)
        state = await _state(client, gid)
        attacker = _melee_units(state, human)[0]
        target = _enemy_units(state, human)[0]

        from app.database import AsyncSessionLocal
        from app.models import Unit
        async with AsyncSessionLocal() as s:
            a = await s.get(Unit, attacker["id"])
        ...
        assert body["attacker_unit_id"] == attacker["id"]
        assert body["target_unit_id"] == target["id"]
        assert body["damage"] > 0
        assert body["target_hp_after"] == max(0, 20 - body["damage"])
        assert body["counter_damage"] >= 0
        assert "预计伤害" in body["description"]
        assert "反击" in body["description"]

    async def test_forecast_attack_does_not_mutate_hp_or_action_state(self, client):
        gid, human, _ai = await _make_game(client)
        state = await _state(client, gid)
        attacker = _melee_units(state, human)[0]
        target = _enemy_units(state, human)[0]
        ...
        before = await _state(client, gid)
        before_attacker = next(u for u in _my_units(before, human) if u["id"] == attacker["id"])
        before_target = next(u for u in _enemy_units(before, human) if u["id"] == target["id"])
        ...
        after = await _state(client, gid)
        after_attacker = next(u for u in _my_units(after, human) if u["id"] == attacker["id"])
        after_target = next(u for u in _enemy_units(after, human) if u["id"] == target["id"])
        assert after_attacker["hp"] == before_attacker["hp"]
        assert after_attacker["has_acted"] == before_attacker["has_acted"]
        assert after_attacker["mp"] == before_attacker["mp"]
        assert after_target["hp"] == before_target["hp"]

class TestAttackOutcomeRules:
