extends Node
## game_overview.gd — comprehensive UI screenshot for visual review.
## Shows the GAME VIEW with EVERY overlay in a sensible layout:
##   - Board (left-center)
##   - InfoPanel (left 30%: commander + unit + players)
##   - HUD 4-corner pills (turn/phase, player/end-turn, gold/CO, AI/war-report)
##   - ActionBubble (visible, mid-board) — simulating "unit selected" state
## WarReportPanel is intentionally hidden so it doesn't cover the action bubble.

const _OUT_PATH := "user://game_overview.png"
...
func _ready() -> void:
	...
	if unit_info_node != null:
		unit_info_node.bbcode_enabled = true
		unit_info_node.text = "[color=#c9a14a][b]Lv.3 剑士 (Swordsman)[/b][/color]\n" \
	...
	if players_node != null:
		players_node.text = "[color=#e85a6a]🔴 学长[/color] — 3 单位 · 💰 500 · 🟢 行动中\n" \
	...
	# ActionBubble — show this one (player selected a unit).
	...
	img.save_png("res://game_overview.png")
	print("Saved overview → %s" % _OUT_PATH)
	get_tree().quit(0)