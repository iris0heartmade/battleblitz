extends Control
## in_progress_controller.gd — 进行中视图控制器(#16 新增, #17 收窄语义)。
##
## 跟 main menu 的 ResumeButton 不同:ResumeButton 只显示"最近一个可恢复"的一键按钮;
## InProgressView 列出**存档域**里所有可恢复的项目(中断存档 / 已存的活动主线 manual 槽),
## 每项独立操作。**不**渲染 /games 里的活动对局 — 活动对局走联机大厅的 room list。
## 配色与 SavesView 一致:暗背景 + 金色边框 + 白/暗黄文字。
##
## 对外接口:
##   open()                       — main 切到 in_progress view 后调用:拉 /saves
...
# State
# InProgressView 语义:**只渲染存档域**。
# - suspend:战斗中按暂停写入的临时进度,设计上至多 1 个。
# - 活动主线 manual 槽:已经手动存过档、还活着的活动主线 mainline_id != "" 的记录。
# 故意不渲染 /games 里的活动对局 — 那跟"中断存档"是两回事,
# 用户应通过联机大厅的 room list 找活动对局(再入/观战入口已在 lobby)。
...
func _refresh() -> void:
	...
	_render_empty()
	# 只拉 /saves(存档域);不拉 /games — 活动对局不归本视图管。
...
func _render_full() -> void:
	...
	if not _suspend_record.is_empty():
		_render_section_title("⏸  中断存档  (战斗中保存的临时进度 · 至多 1 个)")
		_render_suspend_row()
		total += 1
	...
	if total == 0:
		var empty := Label.new()
		empty.text = "(无中断存档或活动主线存档 — 战斗中按暂停会写入中断存档;通关或手动存的活动主线会显示在这里)"
