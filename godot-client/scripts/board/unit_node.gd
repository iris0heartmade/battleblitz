...
##
## 数据字典由 Board._rebuild_units 传入,典型 shape:
## {
##   id: int, type: String ("infantry"/"knight"/"mage"/"healer"/...),
##   color: String ("red"/"blue"/"green"/"yellow"),
##   level: int (1-3), hp: int, max_hp: int,
##   mp: int (默认 0), max_mp: int (默认 100,skill 单位才有),
##   morale: int (0-5), has_acted: bool, has_moved: bool,
##   x: int, y: int
## }
...
func _build_pieces(team_color: Color) -> void:
	# ---- 中心 marker(主色块 + 边框 + 单字母 type) ----
	_marker = ColorRect.new()
	_marker.size = Vector2(36, 36)
	_marker.position = -_marker.size * 0.5
	_marker.color = team_color
	_marker.mouse_filter = Control.MOUSE_FILTER_IGNORE
	add_child(_marker)

	_type_label = Label.new()
	_type_label.text = String(unit_data.get("type", "?")).left(1).to_upper()
	_type_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_type_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	_type_label.size = _marker.size
	...
	_type_label.add_theme_color_override("font_shadow_color", Color(0, 0, 0, 0.7))
	...
	_mp_badge.position = Vector2(_marker.size.x * 0.5 - 6, -_marker.size.y * 0.5)
	...
	_mp_badge_label.size = Vector2(18, 12)
	_mp_badge_label.position = _mp_badge.position + Vector2(-3, -2)
	...
	_star_label.position = Vector2(-_marker.size.x * 0.5 - 2, -_marker.size.y * 0.5 - 4)
	...
	_acted_overlay.size = _marker.size
