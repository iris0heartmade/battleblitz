...
| 地图编辑器 | 地形部署、地表部署、单位部署、单位/建筑归属、生态切换、尺寸、撤销/重做、保存/加载/删除可用 | fill、line、select/move/recolor 等高级工具待补 |
...
- Godot 导入和 smoke test 均已覆盖本轮动态按钮逻辑、地图编辑器撤销/重做、三部署模式、地表归属和生态切换。当前 smoke test 为 `Passed: 217 Failed: 0`，额外包含初始指令气泡、移动后气泡、攻击可用状态、主动治疗技能、占领按钮、编辑器撤销、编辑器重做、地形/地表/单位部署等断言。
...
### 2026-07-19 追加进展：地图编辑器三部署模式

- 编辑器模式拆为“地形部署 / 地表部署 / 单位部署”。
- 地形部署只处理基础地形和生态视觉分支；“套用”生态按钮会立即更新当前地图 `biome`。
- 地表部署支持城堡、村庄、兵营、城门，并可设置无主、红方、蓝方、绿方、黄方归属。
- 单位部署保留单位类型、等级和四队归属。
- 后端自定义地图新增 `tile_owners` 字段，开局时按颜色解析成实际玩家 `owner_id`，旧地图缺省为空数组。
