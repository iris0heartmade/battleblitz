...
- Map editor first slice: menu entry, dedicated editor view, board-backed preview, resize controls, terrain/biome controls, click-to-paint terrain, unit placement/removal, saved-map listing/loading/deletion, immediate `custom:{id}` lobby preset upsert, and `/editor/maps` network wrappers.
...
1. **Map editor is only partially ported.**
   Godot now has an editor entry, panel, board-backed preview, resize controls, terrain/biome controls, click-to-paint terrain, unit placement/removal, saved-map listing/loading/deletion, immediate `custom:{id}` lobby preset upsert, and editor API wrappers. It still needs undo/redo, unit move polish, and richer WebUI-style editing workflows.
...
1. Finish the Godot map editor first slice: add undo/redo and unit move polish.
