...
- Map editor first slice: menu entry, dedicated editor view, board-backed 15x15 preview, terrain/biome controls, click-to-paint terrain, and `/editor/maps` network wrappers.
...
1. **Map editor is only partially ported.**
   Godot now has an editor entry, panel, board-backed preview, terrain/biome controls, click-to-paint terrain, and editor API wrappers. It still needs saved-map loading, delete, unit placement, resize, undo/redo, and richer WebUI-style editing workflows.
...
1. Finish the Godot map editor first slice: load saved custom maps, delete custom maps, and confirm saved `custom:{id}` maps appear in lobby presets.
