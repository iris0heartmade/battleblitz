...
- Map editor first slice: menu entry, dedicated editor view, board-backed 15x15 preview, terrain/biome controls, click-to-paint terrain, unit placement/removal, saved-map listing/loading/deletion, immediate `custom:{id}` lobby preset upsert, and `/editor/maps` network wrappers.
...
1. **Map editor is only partially ported.**
   Godot now has an editor entry, panel, board-backed preview, terrain/biome controls, click-to-paint terrain, unit placement/removal, saved-map listing/loading/deletion, immediate `custom:{id}` lobby preset upsert, and editor API wrappers. It still needs resize, undo/redo, unit move polish, and richer WebUI-style editing workflows.
...
1. Finish the Godot map editor first slice: add unit move polish, then resize and undo/redo.
