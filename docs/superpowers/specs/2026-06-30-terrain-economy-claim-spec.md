...
- **招募交互**：
  - 鼠标点击己方佣兵站 → 弹出招募 UI（不是走单位行动气泡）
  - 招募 UI 显示可购买单位列表 + 价格 + 当前金币
  - 点击某单位 + 确认 → `POST /recruit` → 单位生成在佣兵站所在格
  - **新单位出现在佣兵站上，且 `has_acted = True`、`has_moved = True`、MP=0**（本回合不可行动，标"已行动"dim）
  - 当佣兵站上**已有单位**：鼠标点击该格优先选中单位（鼠标操作单位优先级高于操作佣兵站）
  - 当佣兵站不归操作者所有：点击时 toast 提示"该佣兵站属于玩家 {user_name}，无法招募"，不弹招募 UI
