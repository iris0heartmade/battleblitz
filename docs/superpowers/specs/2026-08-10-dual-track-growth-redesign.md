...
本版在第一版基础上整体抬高成长率，避免角色 Lv20 仍像缺饭吃。

| 规则 | 数值处理 | 说明 |
|---|---|---|
| 整体抬升 | 非 MOV 成长率整体约 +10% | 所有人都吃饱一点 |
| 强侧强化 | 角色/职业主轴额外 +5% | 剑士更会砍，法师更会烧 |
| 弱侧攻击 | 弱攻击不再为 0，通常给 35% | 物理职业也能长一点魔攻，魔法职业也能长一点物攻 |
| 普通侧下限 | 正常参与战斗的非 MOV 项不低于 35% | 低于这个就不是成长，是摆烂 |
| 强概念 / T2 | 允许成长率超过 100% | `110% = 必定 +1 + 10% 额外 +1`，用于表达概率单级 +2 |
| MOV | 0% | 移动力仍不随机成长 |

### 7.3 英雄成长建议

| Hero | Display | Class | HP | ATK | DEF | MATK | MDEF | 定位 |
|---|---|---|---:|---:|---:|---:|---:|---|
| `yun` | 云 | warlock | 80 | 35 | 35 | 90 | 50 | 高输出法师，物攻不再归零 |
| `yuanying` | 鸢影 | warlock | 85 | 35 | 40 | 80 | 55 | 稳定型法师，生存略好，名称以“鸢影”为准 |
| `anna` | 安娜 | healer | 80 | 35 | 45 | 50 | 70 | 防守型治疗者，MDEF 是强侧 |
| `youko` | 洋子 | bard | 75 | 35 | 35 | 55 | 65 | 支援/歌者，魔防和功能性突出 |
| `lin_yilan` | 林依澜 | bard | 75 | 35 | 45 | 35 | 70 | 战术型歌者，弱魔攻仍保留成长下限 |
| `baiyu` | 白予 | sage | 65 | 35 | 35 | 100 | 75 | 高概念法师，MATK 强侧接近单级稳定成长 |

### 7.4 职业成长建议

这些成长率只服务于敌方普通兵 autolevel、自由模式标准 Lv10 基准推导、以及图表校验。己方佣兵仍不掷成长、不拿经验条。

| Class | Tier | HP | ATK | DEF | MATK | MDEF | MOV | 定位 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| swordsman | 1 | 95 | 60 | 45 | 35 | 35 | 0 | 标准近战 |
| archer | 1 | 75 | 70 | 35 | 35 | 35 | 0 | 远程高攻低防 |
| knight | 1 | 100 | 55 | 60 | 35 | 35 | 0 | 重甲，低机动 |
| lancer | 1 | 85 | 60 | 40 | 35 | 35 | 0 | 高机动枪兵 |
| warrior | 1 | 105 | 75 | 35 | 35 | 35 | 0 | 高 HP 高攻低防，HP 有小概率 +2 |
| warlock | 1 | 70 | 35 | 35 | 75 | 45 | 0 | 魔法输出 |
| healer | 1 | 75 | 35 | 35 | 45 | 60 | 0 | 治疗/低输出 |
| bard | 1 | 70 | 35 | 35 | 40 | 60 | 0 | 支援/脆弱 |
| dragon_rider | 1 | 95 | 65 | 40 | 35 | 35 | 0 | 飞行压制 |
| falcon_knight | 1 | 80 | 55 | 35 | 35 | 50 | 0 | 飞行高速/低耐久 |
| blade_master | 2 | 90 | 70 | 50 | 40 | 40 | 0 | T2 剑术爆发 |
| sniper | 2 | 80 | 80 | 40 | 40 | 35 | 0 | T2 远程专精 |
| paladin | 2 | 100 | 65 | 55 | 35 | 40 | 0 | T2 均衡前排 |
| sage | 2 | 75 | 40 | 40 | 85 | 65 | 0 | T2 魔法核心 |
| saint | 2 | 80 | 40 | 45 | 60 | 75 | 0 | T2 治疗/魔防核心 |
| berserker | 2 | 110 | 85 | 40 | 35 | 35 | 0 | T2 狂战士，HP 稳定有概率 +2 |
...
2026-08-10 按 Lv10 英雄 vs T1 普通佣兵击杀次数回看后，生产数据做小幅校准。目标不是重做公式，而是压低普通佣兵中的尖峰输出，同时让治疗师、吟游诗人不再只能象征性刮痧。

| Class | 调整 | 目标手感 |
|---|---|---|
| healer | `base_matk 8 -> 12`, `matk growth 25 -> 45` | 打 Lv10 英雄约 7-8 次击杀 |
| bard | `base_matk 10 -> 12`, `matk growth 20 -> 45`, `mov growth 5 -> 0` | 打 Lv10 英雄约 6-7 次击杀，保留支援定位 |
| archer | `base_atk 20 -> 18`, `atk growth 50 -> 45` | 长射程仍危险，但不再过快压死英雄 |
| knight | `base_atk 22 -> 20`, `atk growth 45 -> 35` | 双连击前排偏肉，不兼任最高单体输出 |
| swordsman | `atk growth 45 -> 40` | 标准近战略降英雄压迫 |
| lancer | `atk growth 45 -> 40` | 机动换输出，避免轻骑过全能 |
| warrior | `atk growth 55 -> 50` | 保留重击，但削一点成长尖峰 |
| dragon_rider | `base_atk 16 -> 15`, `atk growth 50 -> 45` | 飞行/连击价值仍在，直接击杀速度放缓 |
| falcon_knight | `atk growth 35 -> 30` | 高机动单位不靠纯攻击压人 |

校验口径：无暴击、无士气、无地形、无反击，使用当前生产职业基础值 + 成长期望计算 Lv10。验收测试见 `game/tests/test_combat_pacing_calibration.py`。

### 7.7 会话回顾补漏

2026-08-10 复盘后补齐以下实现约束，避免设计停在纸面：

| 漏项 | 修正 | 验收测试 |
|---|---|---|
| 生产职业表仍有 5/15/20/25/30 成长 | 所有非 MOV 成长率统一不低于 35% | `test_non_mov_class_growth_rates_respect_design_floor` |
| 多个职业仍有 MOV 成长 | 所有 `class_growth_rates["mov"]` 清零 | `test_class_mov_growth_rates_are_static` |
| 英雄仍走 `职业成长 + personal_growth_modifier` | `HeroProfile` 增加 `character_growth_rates`，英雄策略优先读取完整角色成长表 | `test_*_growth_rates_are_independent_*` |
| 自由模式英雄会被 override 回 L1 数值 | 无主线 `campaign_state` 时按当前 `level` 和英雄成长表生成标准英雄数值 | `test_free_mode_hero_override_keeps_standard_l10_growth` |
| 自由模式生成缺稳定 seed | 新增 `_spawn_growth_seed(game_id, seat, unit_index, unit_type)` | `test_spawn_growth_seed_is_stable_and_slot_specific` |
