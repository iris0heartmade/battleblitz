# BattleBlitz Godot 48x48 地图表现规范

- 日期：2026-07-14
- 分支：`godot-map-port`
- 范围：仅 Godot 前端地图表现层；不修改 Python 后端规则、协议、路由和数据结构
- 目标：为 BattleBlitz 建立一套可扩展的 48x48 Godot 战棋地图客户端基座
...
2. 用 Godot 4.7 将地图改造为 48x48 网格渲染体系。
...
3. 不承诺本轮完成完整对战流程。
4. 不要求本轮重做整套正式美术资源，只要求现有 48x48 素材体系能支撑新渲染体系。
...
地图标准 tile 基准统一定义为 `48x48`。这是渲染层规范，不要求后端地图坐标、地图 JSON 或规则计算发生任何变化。
...
1. `MapMetrics`
   统一维护 `tile_size = 48`、`cell_to_pixel`、`pixel_to_cell`、地图像素矩形边界和单元格中心点换算。
2. `MapTheme`
...
本轮采用“结构先到位，素材尽量复用现有 48x48 体系”的策略：

1. 先建立正式 `48x48` 渲染规范。
2. 现有 `48x48` PNG、FE8 `16x16` atlas 和占位素材允许通过整数倍对齐、裁切和 fallback 规则进入 48x48 体系。
3. 后续替换正式美术时，不应改地图加载逻辑和场景结构，只更新 `MapTheme`/TileSet 资源。

### 4.2 48x48 资源规则

1. 所有地图格子的逻辑尺寸固定为 `48x48`。
2. 现有 `48x48` 资产视为正式基础尺寸，不再为了统一规格而额外放大。
3. FE8 `16x16` atlas 若继续使用，优先按整数倍扩成 `48x48` 等效切片，不使用双线性缩放。
...
1. `TileSetBuilder.TILE_SIZE = 48` 从“原型常量”升级为“正式规范常量”，并统一收口到公共度量层。
...
1. 现有 48x48 资产的层次感和装饰密度可能不够，这属于已接受的阶段性限制。
...
这份规范的核心结论只有一句：先把 BattleBlitz 的 Godot 地图前端从“能画 JSON 的原型”提升成“48x48、多层、可扩展的战棋地图基座”，素材可以复用，结构不能再凑合。
