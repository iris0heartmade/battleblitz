...
> 当前架构更新（2026-07-13）：P2.6 地图自带 `initial_units` 已成为当前创建战斗路径；指挥官 / CO 选择已覆盖 `app/commanders/`、`routes/commanders.py`、创建游戏 commander 字段、`GameStateOut.co_states`、大厅 UI 与 HUD 渲染；AI 攻击路径已补齐同玩家攻击一致的反击分支，避免同一战斗规则在 AI/玩家间漂移。
...
- ✅ **数据驱动地图（P2.6）**：42+ 张内置图，每张 JSON 自带 `initial_units` 数组；启动时硬校验完整性；老格式图自动按 `_classic_initial_units()` 回退。详见 [`maps.md`](../参考/地图JSON规范.md)。
- ✅ **占领 / 招募 / 金币经济**：village 50g/turn、barracks 100g/turn、castle_vault 150g/turn，barracks 可招募剑/弓/骑/疗。
- ✅ **初始金币（P2.6 修复）**：`start_game` 给所有非旁观玩家立即发放一次收入；修了一个 missing `session.flush()` bug。

### 0.2 进行中（🚧）

- 🚧 生产环境 WebSocket 网关（前端 `app.js` 仍用 3s 轮询；debug_ws 端点已可用，但未对接客户端）
- 🚧 AI reaction 演出优化（DB 已存 reaction，前端展示链路半成品）
- 🚧 死代码清理（[`死代码.md`](../维护/死代码清单.md) 列了 5 个 dead config + 11 个 dead import + 6 个 dead method）
...
- [`maps.md`](../参考/地图JSON规范.md) —— 地图 JSON 规范 + 初始单位布置 + 字符地形表 + 老格式回退。
- [`路线.md`](../路线/路线.md) —— 版本里程碑。
- [`阶段.md`](阶段.md) —— 各开发阶段。
...
| morale | INT | 士气 (0-3)，**新单位默认 0**，击杀+1，提供 ATK/DEF 加成 |
| x / y | INT | 棋盘坐标 |
...
- **新生成的单位默认 0 星士气** — 第一次进入游戏时单位下方的金星星标显示为 `★☆☆`
- 击杀即获得 1 星士气（上限 3），永久保留
- 士气提供百分比攻防加成（每星 +10% ATK / +5% DEF），比线性的等级系统更有"滚雪球"叙事感
- EXP/Level 系统仍保留用于向后兼容
- P2.6+ — 金星星标 UI 之前因为 `.unit { overflow: hidden }` 的剪裁一直被隐藏；现已将 `<div class="morale-stars">` 挂到 cell 而非 unit 上，三颗星能正常显示
...
```json
{
  ...
  "title": "钢铁叛乱",
  "battles": [
    { "id": "battle_01", "map": "mountain_pass",
      "win_condition": "rout", "next": "battle_02" },
    { "id": "battle_02", "map": "river_crossing",
      "win_condition": "rout" }
  ],
  "dialogues": {
    "intro": [
