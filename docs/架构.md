...
- ✅ **数据驱动地图（P2.6）**：42+ 张内置图，每张 JSON 自带 `initial_units` 数组；启动时硬校验完整性；老格式图自动按 `_classic_initial_units()` 回退。详见 [`maps.md`](maps.md)。
...
- [`maps.md`](maps.md) —— 地图 JSON 规范 + 初始单位布置 + 字符地形表 + 老格式回退。
- [`路线.md`](路线.md) —— 版本里程碑。
- [`阶段.md`](阶段.md) —— 各开发阶段。
